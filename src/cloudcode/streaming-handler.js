...
export async function* sendMessageStream(anthropicRequest, accountManager, fallbackEnabled = false) {
            ...
            if (accountManager.isAllRateLimited(model)) {
                ...
                if (minWaitMs > MAX_WAIT_BEFORE_ERROR_MS) {
                    ...
                    if (fallbackEnabled) {
                        ...
                        if (fallbackModel) {
                            logger.warn(`[CloudCode] All accounts exhausted for ${model} (${formatDuration(minWaitMs)} wait). Attempting dynamic fallback to ${fallbackModel} (streaming)`);
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        if (response.status === 429) {
                            ...
                            logger.info(`[CloudCode] 429 (${category} -> ${decision.kind}): ${decision.reason} (${account.email}) [stream]`);
    ...
    if (fallbackEnabled) {
        const fallbackModel = getFallbackModel(model, null, (m) => !accountManager.isAllRateLimited(m));
        if (fallbackModel) {
            logger.warn(`[CloudCode] All retries exhausted for ${model}. Attempting dynamic fallback to ${fallbackModel} (streaming)`);
            const fallbackRequest = { ...anthropicRequest, model: fallbackModel };
            yield* sendMessageStream(fallbackRequest, accountManager, false);
            return;
        }
